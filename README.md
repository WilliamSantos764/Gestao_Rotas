...
## V5.30 — embarque automático por rota
...
## V5.30 — dashboard com detalhamento por dia, cidade e pedido
...
## V5.30 — análise inteligente de rotas
...
As recomendações são apenas sugestões. A V5.30 **não move pedidos automaticamente** entre embarques.
...
## V5.30 — mapa semanal e antecipação de rotas futuras
...
## V5.30 — leite, derivados e filtros de alocação

A V5.30 classifica cada pedido pelos itens que existem dentro dele.
...
## V5.30 — regras selecionáveis de alocação
...
## V5.30 — recálculo automático ao mudar as regras
...
## V5.30 — mapeamento automático de colunas
...
## V5.30 — identificação de planilha otimizada
...
## V5.30 — correção do travamento em “Identificando estrutura”
...
## V5.30 — correção do erro `unzip is not defined`

Foi removida a função `parseWorkbook()` antiga que permaneceu duplicada na V5.30.

Ela sobrescrevia o leitor novo e tentava executar `unzip()`, função que já havia sido substituída pelo leitor seguro.

A V5.30 mantém somente o leitor novo baseado em:
- `buildZipIndex`;
- `extractZipEntry`;
- `DecompressionStream`;
...
## V5.30 — leitor Excel robusto com fallback direto
...
## V5.30 — correção de `parseSharedBytes is not defined`
...
## V5.30 — suporte a XML com namespace/prefixo
...
A V5.30 reconhece os dois formatos.
...
## V5.30 — cálculo do peso por pedido e alocação por capacidade
...
## V5.30 — leite puro somente manual
...
## V5.30 — roteirização porta a porta por embarque
...
## V5.30 — rotas de entrega por motorista na Análise inteligente
...
## V5.30 — perto para longe + App do Motorista
...
## V5.30 — Apps dos Motoristas vinculados ao Gestão Logística
...
## V5.30 — WhatsApp funciona também com o sistema aberto localmente
...
## V5.30 — um único App com todas as entregas do motorista
...
- links V5.30 com múltiplas rotas (`v:2`);
- links antigos com uma rota (`v:1`), convertidos automaticamente.

## V5.30 — GPS do motorista + monitoramento em tempo real
...
## V5.30 — Firebase já integrado ao software
...
## Correção V5.30 — link do App do Motorista
