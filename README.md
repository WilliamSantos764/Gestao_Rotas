...
## V5.32 — embarque automático por rota
...
## V5.32 — dashboard com detalhamento por dia, cidade e pedido
...
## V5.32 — análise inteligente de rotas
...
As recomendações são apenas sugestões. A V5.32 **não move pedidos automaticamente** entre embarques.
...
## V5.32 — mapa semanal e antecipação de rotas futuras
...
## V5.32 — leite, derivados e filtros de alocação

A V5.32 classifica cada pedido pelos itens que existem dentro dele.
...
## V5.32 — regras selecionáveis de alocação
...
## V5.32 — recálculo automático ao mudar as regras
...
## V5.32 — mapeamento automático de colunas
...
## V5.32 — identificação de planilha otimizada
...
## V5.32 — correção do travamento em “Identificando estrutura”
...
## V5.32 — correção do erro `unzip is not defined`

Foi removida a função `parseWorkbook()` antiga que permaneceu duplicada na V5.32.

Ela sobrescrevia o leitor novo e tentava executar `unzip()`, função que já havia sido substituída pelo leitor seguro.

A V5.32 mantém somente o leitor novo baseado em:
- `buildZipIndex`;
- `extractZipEntry`;
- `DecompressionStream`;
...
## V5.32 — leitor Excel robusto com fallback direto
...
## V5.32 — correção de `parseSharedBytes is not defined`
...
## V5.32 — suporte a XML com namespace/prefixo
...
A V5.32 reconhece os dois formatos.
...
## V5.32 — cálculo do peso por pedido e alocação por capacidade
...
## V5.32 — leite puro somente manual
...
## V5.32 — roteirização porta a porta por embarque
...
## V5.32 — rotas de entrega por motorista na Análise inteligente
...
## V5.32 — perto para longe + App do Motorista
...
## V5.32 — Apps dos Motoristas vinculados ao Gestão Logística
...
## V5.32 — WhatsApp funciona também com o sistema aberto localmente
...
## V5.32 — um único App com todas as entregas do motorista
...
- links V5.32 com múltiplas rotas (`v:2`);
- links antigos com uma rota (`v:1`), convertidos automaticamente.

## V5.32 — GPS do motorista + monitoramento em tempo real
...
## V5.32 — Firebase já integrado ao software
...
## Correção V5.32 — link do App do Motorista
...
## V5.32 — traçado da rota no mapa do motorista
...
`V5.32 — traçado rodoviário`

## V5.32 — PWA instalável, GPS obrigatório e mapa estável
