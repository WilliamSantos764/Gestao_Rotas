...
## V5.50 — embarque automático por rota
...
## V5.50 — dashboard com detalhamento por dia, cidade e pedido
...
## V5.50 — análise inteligente de rotas
...
As recomendações são apenas sugestões. A V5.50 **não move pedidos automaticamente** entre embarques.
...
## V5.50 — mapa semanal e antecipação de rotas futuras
...
## V5.50 — leite, derivados e filtros de alocação

A V5.50 classifica cada pedido pelos itens que existem dentro dele.
...
## V5.50 — regras selecionáveis de alocação
...
## V5.50 — recálculo automático ao mudar as regras
...
## V5.50 — mapeamento automático de colunas
...
## V5.50 — identificação de planilha otimizada
...
## V5.50 — correção do travamento em “Identificando estrutura”
...
## V5.50 — correção do erro `unzip is not defined`

Foi removida a função `parseWorkbook()` antiga que permaneceu duplicada na V5.50.

Ela sobrescrevia o leitor novo e tentava executar `unzip()`, função que já havia sido substituída pelo leitor seguro.

A V5.50 mantém somente o leitor novo baseado em:
- `buildZipIndex`;
- `extractZipEntry`;
- `DecompressionStream`;
...
## V5.50 — leitor Excel robusto com fallback direto
...
## V5.50 — correção de `parseSharedBytes is not defined`
...
## V5.50 — suporte a XML com namespace/prefixo
...
A V5.50 reconhece os dois formatos.
...
## V5.50 — cálculo do peso por pedido e alocação por capacidade
...
## V5.50 — leite puro somente manual
...
## V5.50 — roteirização porta a porta por embarque
...
## V5.50 — rotas de entrega por motorista na Análise inteligente
...
## V5.50 — perto para longe + App do Motorista
...
## V5.50 — Apps dos Motoristas vinculados ao Gestão Logística
...
## V5.50 — WhatsApp funciona também com o sistema aberto localmente
...
## V5.50 — um único App com todas as entregas do motorista
...
- links V5.50 com múltiplas rotas (`v:2`);
- links antigos com uma rota (`v:1`), convertidos automaticamente.

## V5.50 — GPS do motorista + monitoramento em tempo real
...
## V5.50 — Firebase já integrado ao software
...
## Correção V5.50 — link do App do Motorista
...
## V5.50 — traçado da rota no mapa do motorista

O App do Motorista agora desenha a rota rodoviária no próprio mapa.

Ao tocar em `Ativar GPS e roteirizar`:

1. o navegador obtém a localização atual do caminhão;
2. o App considera todas as entregas ainda pendentes da rota ativa;
3. ordena as paradas da mais próxima para a mais distante por estrada;
4. consulta o OSRM para obter o caminho real pelas ruas;
5. desenha uma linha azul tracejada:
   `posição atual → cliente 1 → cliente 2 → ... → última entrega`;
6. destaca a próxima entrega;
7. centraliza o mapa no percurso atual;
8. recalcula automaticamente após deslocamento relevante, com intervalo controlado para não sobrecarregar o serviço de rotas;
9. ao marcar uma entrega como concluída, remove-a do próximo cálculo e redesenha o caminho.

As demais viagens/dias continuam visíveis no App, mas o mapa com GPS ativo dá foco à rota que deve ser executada agora.

### Publicação

Atualize no GitHub Pages:
...
`V5.50 — traçado rodoviário`

## V5.50 — PWA instalável, GPS obrigatório e mapa estável

### 1. Linha da rota contínua

A rota no mapa do motorista agora é uma linha azul contínua.
O traçado continua seguindo a geometria rodoviária retornada pelo OSRM.

### 2. Mapa sem piscar

O Leaflet não é mais destruído e recriado em cada atualização do GPS.

Agora:
- o mapa é criado uma única vez;
- o marcador do caminhão usa `setLatLng`;
- a linha da rota usa `setLatLngs`;
- as tiles permanecem carregadas;
- atualizações automáticas não ficam forçando zoom;
- o mapa só ajusta o enquadramento quando necessário.

### 3. GPS obrigatório

Depois que as viagens são carregadas, uma tela de bloqueio exige autorização do GPS.

Enquanto não houver uma posição válida:
- as entregas ficam bloqueadas;
- a rota não pode ser utilizada;
- o App informa se a permissão foi negada;
- se a permissão já tiver sido concedida anteriormente, o GPS é iniciado automaticamente.

O navegador continua sendo o responsável pela permissão; o sistema não consegue contornar uma negação do usuário.

### 4. App instalável no celular

Foi criada uma PWA com:
- `manifest.webmanifest`;
- `sw.js`;
- ícones 192x192 e 512x512;
- botão `Instalar App no celular`;
- suporte ao prompt de instalação do Android/Chrome;
- instrução de `Adicionar à Tela de Início` para iPhone/iPad;
- modo `standalone`;
- abertura pelo ícone instalado.

O App salva no próprio aparelho o último `appId` do motorista. Assim, quando for aberto pelo ícone instalado, ele recupera as viagens do motorista no Firebase.

### Arquivos novos que precisam ir para o GitHub Pages
...
Todos devem preservar a mesma estrutura de pastas deste ZIP.

### Observação sobre rastreamento
...
## V5.50 — canhoto obrigatório e WhatsApp

A entrega só pode ser concluída depois de fotografar e salvar o canhoto.

Fluxo:
1. GPS ativo.
2. `Tirar foto do canhoto`.
3. Conferir a imagem.
4. Enviar ao Firebase Storage.
5. Registrar entrega no Realtime Database.
6. Marcar a parada como concluída.
7. Recalcular a rota.
8. Abrir o WhatsApp escolhido na central com dados da entrega e link da foto.

Na central, configure:
`Análise inteligente de rotas → Rastreamento em tempo real → WhatsApp que recebe os canhotos de entrega`.

Se o número estiver vazio, a entrega fica bloqueada.

### Importante sobre WhatsApp
O App abre automaticamente a conversa e preenche a mensagem. O WhatsApp não permite que um site/PWA pressione o botão Enviar automaticamente. Para envio sem nenhum toque final é necessário WhatsApp Business Cloud API + backend.

### Firebase Storage
Ative Firebase Storage e publique `firebase-storage-rules.txt`.
Publique também `firebase-rules.json` atualizado no Realtime Database.


## V5.50 — correção do travamento do canhoto

O canhoto não usa mais Firebase Storage.

A foto é comprimida no celular e gravada no Realtime Database em:
`driverApps/<appId>/deliveryProofs/<proofId>`.

A entrega só é concluída depois que o App lê o canhoto de volta e confirma que a imagem existe.

Há tempo limite na preparação, gravação e confirmação. Se a internet falhar, os botões voltam a funcionar e a entrega permanece pendente.

O WhatsApp recebe um link `canhoto.html`, que mostra a foto, motorista, cliente, pedidos, embarques, data/hora e GPS.

A V5.50 inclui novamente toda a PWA:
- manifest.webmanifest
- sw.js
- icons/
- canhoto.html
...
## V5.50 — finalização das rotas e canhotos na central

A central ganhou o painel `Finalização das rotas e canhotos`.

Para cada motorista e dia, o Gestão Logística mostra:
- PENDENTE;
- NOVO CANHOTO SOLICITADO;
- ROTA FINALIZADA;
- total de entregas;
...
A rota só fica `FINALIZADA` quando todas as paradas possuem canhoto válido.

### Conferência do canhoto na central

Cada entrega com comprovante possui `Ver canhoto`.
A foto abre dentro do próprio Gestão Logística junto com motorista, cliente, rota, pedidos, embarques e GPS.

### Foto ruim / ilegível

Use `Excluir e pedir outra`.

O sistema:
1. apaga a foto do canhoto;
2. remove o status de concluído;
3. grava uma solicitação de nova foto;
4. a rota volta para PENDENTE;
5. o App do Motorista recebe a atualização em tempo real;
6. a entrega mostra `CENTRAL SOLICITOU NOVO CANHOTO`;
7. o motorista precisa fotografar novamente;
8. quando a nova foto é confirmada, a solicitação é removida e a entrega volta a ser concluída.

### Otimização

A central não fica baixando todas as imagens continuamente.
O App grava também `deliveryProofMeta`, sem o base64 da foto.
A central acompanha esses metadados em tempo real e só baixa a imagem completa quando você clica em `Ver canhoto`.

Canhotos antigos da V5.34 são migrados automaticamente para metadados na primeira conexão.

## V5.50 — rotas pendentes e concluídas separadas

### Gestão Logística / central

Quando uma rota chega a 100% de canhotos válidos:
- ela continua salva no histórico;
- continua disponível no painel de Finalização e Canhotos;
- sai automaticamente do mapa operacional;
- sai da lista normal de rotas geradas;
- deixa de aparecer como rota a ser executada.

Para consultar rotas antigas, ative:
`Mostrar rotas concluídas no mapa`.

Quando essa opção está ativa, as rotas concluídas reaparecem em verde com status FINALIZADA.

Se um canhoto for excluído e a central solicitar nova foto, a rota deixa de ser finalizada e volta automaticamente para a visão operacional.

### App do Motorista

O App agora possui duas áreas:

1. `Rotas pendentes para fazer`
   - mostradas primeiro;
   - usadas pelo GPS;
   - usadas para cálculo da próxima entrega;
   - aparecem no mapa por padrão.

2. `Rotas concluídas`
   - ficam em uma área separada e recolhida;
   - continuam mostrando clientes e acesso aos canhotos;
   - não participam mais da navegação;
   - não aparecem no mapa por padrão.

O motorista pode marcar:
`Mostrar rotas concluídas também no mapa`.

Se a central excluir um canhoto e pedir nova foto, a rota sai de Concluídas e volta imediatamente para Pendentes.

## V5.50 — gerar somente rotas pendentes + exceções do motorista

### Geração de rotas

Ao clicar em `Gerar rotas dos motoristas`:
- rotas já FINALIZADAS são ignoradas;
- somente rotas ainda pendentes são recalculadas;
- uma rota pendente mantém o mesmo `generatedAt`, preservando os canhotos já registrados;
- entregas marcadas anteriormente como `Não deu tempo de entregar` são reativadas para nova tentativa quando a rota pendente é gerada novamente;
- o histórico da ocorrência continua no Firebase, marcado como inativo/reactivado.

### Novas opções no App do Motorista

Para uma entrega ainda aberta existem:

1. `📷 Tirar foto do canhoto`
2. `⚠ Canhoto retido`
3. `⏱ Não deu tempo de entregar`

#### Canhoto retido
Significa que a entrega física foi realizada, mas o canhoto ficou retido.
- conta como entrega resolvida operacionalmente;
- sai da navegação;
- vai para a central com motorista, cliente, pedidos, embarques, data/hora, GPS e observação;
- a rota pode ser finalizada, mas a central mostra `FINALIZADA • CANHOTO RETIDO`.

#### Não deu tempo de entregar
- não conclui a entrega;
- envia a ocorrência para a central;
- a parada sai do trajeto atual do motorista;
- a rota permanece PENDENTE;
- ao gerar novamente a rota pendente na central, essa entrega é reativada para nova tentativa.

O motorista também possui `Voltar para entregar` caso tenha marcado `Não deu tempo` por engano.

### Dados no Firebase

Dentro de `driverApps/<appId>`:
`deliveryEvents/<id>` registra CANHOTO_RETIDO e NAO_DEU_TEMPO.

As regras atuais de `driverApps/$appId` já abrangem esse caminho.


## V5.50 — ESPD fonte mestre + reentregas
...
A aba Cliente → Cidade foi removida da navegação.

Canhoto retido e Não deu tempo exigem aprovação da central e data de reentrega. Canhoto retido exige selecionar produtos/quantidades. A reentrega é inserida no planejamento da data escolhida como REENTREGA-<eventId> e aparece em lembretes no Dashboard/Central.


### Fluxo operacional de reentrega

1. Motorista registra `Canhoto retido` ou `Não deu tempo de entregar`.
2. A ocorrência chega na Central de Rotas como `AGUARDANDO APROVAÇÃO`.
3. A central escolhe a data.
4. Canhoto retido obriga selecionar produto(s)/quantidade(s).
5. O sistema cria `REENTREGA-<eventId>` no planejamento da data escolhida.
6. Dashboard mantém lembrete até a reentrega.
...
## V5.50 — roteirização de teste com coordenadas fixas

Para eliminar falhas de geocodificação durante testes, a V5.50 reconhece duas novas colunas opcionais:

- `Latitude Teste`
- `Longitude Teste`

Quando existem coordenadas válidas, elas são usadas diretamente na roteirização e o sistema não consulta Nominatim para aquela parada.

Prioridade de localização:
1. latitude/longitude do ESPD;
2. endereço completo;
3. ponto de entrega + cidade;
4. centro da cidade.

As paradas também passam a ser consolidadas primeiro por `ID Parada Teste`, evitando que endereços semelhantes sejam unidos incorretamente.
...
## V5.50 — descarga na frente do estabelecimento / otimização por rua

A roteirização agora trata cada cliente como uma parada de descarga de caminhão.

### Ponto de descarga na via
- tenta localizar a fachada/endereço do estabelecimento;
- usa o serviço `nearest` do OSRM para encaixar a parada na via trafegável;
- salva o nome da rua;
- utiliza o ponto encaixado na rua na matriz de distâncias, rota final, mapa e App do Motorista;
- a rota final tenta `approaches=curb` para aproximar a chegada pelo lado da via; há fallback automático caso o servidor público não aceite a opção.

### Otimização por rua
A sequência agora considera:
- distância real pelas ruas;
- permanência na mesma rua quando existem várias entregas próximas;
- agrupamento de clientes da mesma via;
- menor quantidade de entradas/saídas repetidas da rua;
- menor retorno em direção à origem;
- continuidade na mesma cidade quando faz sentido.

Isso substitui a regra puramente “mais perto → mais longe” para a operação de distribuição.

### App do Motorista
O recálculo pelo GPS usa a mesma lógica de rua.
`Navegar até esta entrega` passa a mandar o ponto de descarga calculado na via, usando latitude/longitude.
...
## V5.50 — Embarques e Central de Rotas reorganizados
...
## V5.50 — correção da linha azul no App do Motorista

A linha azul contínua foi reforçada para não desaparecer durante sincronizações.

Correções:
- a geometria GPS da rota é preservada quando a central publica uma atualização do mesmo motorista/dia;
- se chegar uma atualização enquanto outro cálculo está rodando, o recálculo fica em fila e executa assim que o anterior terminar;
- rotas com muitas entregas são divididas em blocos menores para evitar limite de waypoints/URL do OSRM;
- a geometria antiga válida não é apagada enquanto a nova está sendo calculada;
- a central envia também uma geometria planejada simplificada para servir como linha imediata/backup;
- em último caso, o App mantém uma linha temporária passando pelas paradas em vez de deixar o mapa sem trajeto;
- ao ativar GPS, a linha planejada é substituída automaticamente pelo trajeto real desde a posição atual.

A organização da V5.41 (Embarques / Central de Rotas) foi mantida sem alterações funcionais.

## V5.50 — Central de Rotas compacta

- Novo botão `Roteirizar todos os caminhões`.
- A fila lateral mostra apenas motoristas ainda sem rota; ao gerar a rota com sucesso, eles desaparecem dessa fila.
- O mapa mantém a altura e ocupa metade da largura no desktop.
- A outra metade contém fila de roteirização, indicadores e motoristas/caminhões ao vivo.
- Motoristas ao vivo são clicáveis; o clique abre os detalhes completos antes exibidos em grandes cartões.
- O painel grande de finalização fica oculto da tela principal, mas seus dados e canhotos permanecem disponíveis no detalhe do motorista.
- A informação técnica do Firebase ficou recolhida em `Configuração técnica do rastreamento`.
- Aprovação manual de reentrega foi removida.
- Canhoto retido e Não deu tempo são registrados automaticamente e o sistema mostra a próxima data/dia/rota fixa da cidade.

## V5.50 — acompanhamento movido para Embarques

- Removido da aba Embarques o bloco `Embarques do próximo dia útil`.
- Para consultar pedidos/itens: `Resumo de embarques da semana → dia → embarque → pedido`.
- O detalhamento continua permitindo ver itens, peso, valor, cliente, cidade, crédito e situação.
- Os cartões completos por motorista/rota (entregas, concluídas, canhoto retido, não entregues e status por cliente) foram movidos para a aba `Embarques`.
- A `Central de Rotas` continua compacta e focada em mapa, roteirização, fila, caminhões ao vivo e ocorrências.
- Nenhuma função de canhoto, rastreamento, rota ou pedido foi removida.

## V5.50 — Situação dos embarques movida para Central de Rotas

Alteração apenas de organização:

- o bloco `Situação dos embarques em entrega` foi retirado da aba `Embarques`;
- o mesmo bloco foi movido para `Central de Rotas`;
- continuam disponíveis motorista, entregas, concluídas, canhoto retido, não entregues, status de cada cliente e acesso aos canhotos;
- nenhuma regra de roteirização, Firebase, GPS, pedidos ou canhotos foi alterada.

## V5.50 — controle de publicação, limpeza operacional e engrenagem

### Regra obrigatória dos Apps
Nenhuma rota e nenhum pedido entra no App do Motorista apenas por importar o ESPD ou por pré-calcular uma rota.

O único comando que libera a operação aos celulares é:

`Roteirizar todos e liberar Apps`

Antes disso:
- os Apps mostram `Aguardando roteirização da central`;
- total de pedidos = 0;
- total de paradas = 0;
- nenhuma linha de trajeto operacional é mostrada;
- envio pelo WhatsApp fica bloqueado.

`Pré-calcular selecionado` serve apenas para conferir uma rota na central e não publica nada.

Quando o ESPD muda, a versão operacional muda e os Apps são bloqueados novamente até nova roteirização geral.

### WhatsApp editável
Em `WhatsApp — envio das rotas` cada motorista possui:
- campo de telefone editável;
- botão `Salvar`;
- status do número;
- botão de envio, liberado somente depois da roteirização geral.

O telefone salvo é usado nos próximos envios e também aparece no cadastro da frota/motorista.

### Desvincular ESPD
`Desvincular e limpar operação` remove:
- pedidos importados;
- embarques;
- pendências;
- rotas geradas da operação;
- cache de mapas/geocodificação;
- rastreamento operacional visível;
- conteúdo de rota/pedidos publicado nos Apps.

São preservados como cadastros mestres:
- Motoristas;
- Frota;
- Rotas Fixas;
- telefones dos motoristas.

Os Apps instalados permanecem com o mesmo vínculo, porém ficam em `Aguardando roteirização`.

### Navegação
Na barra principal ficam visíveis somente:
- `Embarques`
- `Central de Rotas`
- `⚙ Configurações`
...
A V5.50 reduz recomputações repetidas na lista de rastreamento, reaproveitando o agrupamento de status por motorista em uma única passagem. A publicação Firebase também deixa de ocorrer automaticamente a cada abertura da Central de Rotas quando os Apps estão bloqueados.

## V5.50 — matriz rodoviária robusta + liberação segura

### Falha na matriz rodoviária
A matriz deixou de depender de uma única requisição grande.

Nova estratégia:
1. tenta chamada direta em rotas pequenas;
2. divide matrizes maiores em blocos de 14 pontos;
3. cada bloco possui timeout e novas tentativas;
4. se somente um bloco falhar, usa distância geográfica ajustada como fallback técnico naquele bloco;
5. a rota final continua tentando OSRM pelas ruas;
6. rotas finais grandes são divididas em trechos de até 18 pontos;
7. se um trecho do serviço público falhar, o processamento não perde os demais trechos.

O antigo limite rígido de 45 paradas foi elevado para 120.

### Roteirizar todos e liberar Apps
Agora é uma operação `tudo ou nada`.

- processa sempre a semana inteira e todos os motoristas;
- primeiro calcula todas as rotas;
- se qualquer rota tiver pendência, nenhum App é liberado;
- se o Firebase falhar para qualquer motorista, a central devolve todos os Apps para `Aguardando roteirização`;
- somente após todas as publicações serem confirmadas o sistema marca os Apps como liberados.

Isso elimina publicação parcial e o estado em que a central dizia “liberado” apesar de algum motorista ter falhado.

### Indicadores movidos para Embarques
Foram movidos para a aba `Embarques`:
- Embarques abaixo da meta;
- Pedidos compatíveis;
- Pedidos futuros aproveitáveis;
- Sugestões de consolidação;
- Rotas sem peso.

`Embarques abaixo da meta` e `Rotas sem peso` são atualizados automaticamente.
Os três indicadores que dependem de corredor rodoviário exibem `—` antes da análise, evitando um zero falso.
Use `Analisar consolidação` para calcular os valores reais.

A análise também recebeu tentativas automáticas e fallback técnico caso o serviço rodoviário público fique indisponível.

## V5.50 — auditoria de estabilidade e regressões

Esta versão foi fechada após auditoria estática e testes de navegador dos fluxos críticos.

### Falhas corrigidas
- corrigido `lastRouteAnalysisDayKey is not defined`;
- declarados e resetados corretamente `lastRouteAnalysisDayKey` e `lastRouteAnalysisCounts`;
- adicionada a função `norm()` que faltava no App do Motorista;
- removido código residual da antiga aprovação de reentregas;
- republicação Firebase atualiza somente `/bundle`, preservando canhotos, metadados, retakes e ocorrências;
- desvincular ESPD não recria um planejamento vazio e zera os indicadores visuais da operação;
- persistência local e remoção do vínculo do ESPD são best-effort: bloqueio de localStorage/IndexedDB não derruba o software;
- App do Motorista tolera estado sem payload/rota durante sincronizações;
- camadas do mapa do motorista são recriadas se necessário sem destruir o mapa;
- linha azul mantém geometria planejada/live e possui fallback;
- matriz rodoviária do App do Motorista também usa blocos, retries e fallback, além da matriz robusta da central;
- contagem `Não entregues` na situação das rotas usa explicitamente as ocorrências `Não deu tempo`.

### Testes automatizados concluídos
Foram executados testes em Chromium headless com o ESPD de teste do próprio projeto e mocks controlados de OSRM/Firebase:

1. inicialização da Central sem `pageerror`;
2. navegação principal e menu de engrenagem;
3. importação de 176 linhas do ESPD de teste;
4. criação de embarques e pendências;
5. indicadores de consolidação;
6. análise de consolidação;
7. `Roteirizar todos e liberar Apps` com 9 rotas;
8. edição e persistência do WhatsApp do motorista;
9. falha forçada da matriz rodoviária com fallback e continuidade;
10. falha forçada de publicação Firebase com rollback para `Aguardando roteirização`;
11. desvinculação do ESPD com 0 pedidos, 0 planejamentos e 0 linhas visíveis, preservando cadastro de frota/motoristas e Rotas Fixas;
12. App do Motorista em `Aguardando roteirização`, sem pedidos nem paradas;
13. App do Motorista liberado com linha azul planejada e recálculo GPS;
14. App do Motorista com OSRM indisponível, mantendo rota/linha por fallback;
15. visualizador de canhoto tratando link inválido sem erro de execução;
16. persistência local bloqueada sem derrubar a Central.

Também foram executados:
- `node --check` em `index.html`, `motorista.html`, `canhoto.html` e `sw.js`;
- busca TypeScript por identificadores indefinidos (`TS2304`/`TS2552`) — nenhum encontrado;
- verificação de IDs HTML duplicados — nenhum encontrado;
- verificação dos handlers HTML para funções inexistentes — nenhuma encontrada.

## V5.50 — mapa saneado e situação somente no motorista ao vivo

### Mapa do App do Motorista
A Central e o App agora validam coordenadas em duas etapas.

São rejeitados:
- pontos fora dos limites geográficos do Brasil;
- pontos muito distantes da origem operacional em Santa Rita do Sapucaí;
- resultado do OSRM `nearest` excessivamente distante do estabelecimento;
- cache antigo com coordenada fora da área operacional;
- pontos impossíveis em `plannedGeometry` e `liveGeometry`.

O App também:
- não inclui coordenadas inválidas no `fitBounds`;
- não cria marcador fora da área operacional;
- tenta recuperar uma coordenada de via inválida usando a coordenada do estabelecimento;
- usa o endereço textual no Google Maps quando não existir coordenada confiável.

Isso impede o mapa de abrir em escala mundial ou desenhar linha azul atravessando o Atlântico.

### Situação dos embarques
O bloco `Situação dos embarques em entrega` foi retirado da tela principal da Central de Rotas.

Também foram retirados os cartões grandes de resumo das rotas.

Para consultar esses dados:
`Central de Rotas → Motoristas / caminhões ao vivo → clique no motorista`.

O detalhe continua contendo embarques, entregas, concluídas, canhoto retido, não entregues, clientes, pedidos, ocorrências e acesso ao canhoto.

## V5.50 — publicação atômica de todas as rotas + status confirmado

O fluxo `Roteirizar todos e liberar Apps` foi refeito.

### Conferência antes do envio
Para cada motorista/dia o sistema compara:
- embarques atribuídos;
- todos os pedidos desses embarques;
- rota gerada;
- pedidos presentes nas paradas;
- número do embarque;
- versão atual do ESPD.

Se faltar um pedido ou embarque, nenhum App é liberado.

### Publicação única
Os bundles de todos os motoristas são preparados primeiro e enviados ao Firebase com uma única atualização multi-localização em `driverApps`.

Isso evita o cenário em que alguns Apps recebem a nova operação e outros permanecem na anterior.

Depois da escrita, o sistema relê todos os bundles e confirma:
- motorista;
- assinatura da rota;
...
Somente após todas as confirmações a central muda para `Apps liberados`.

### Status por motorista
Cada motorista agora possui estado explícito:
- `AGUARDANDO LIBERAÇÃO`
- `PUBLICANDO`
- `APP LIBERADO`
- `ERRO DE PUBLICAÇÃO`

A área `Motoristas / caminhões ao vivo` também mostra esse status.

### Rotas exibidas pelo App
Após a publicação, a central grava os `publishedDayKeys` de cada motorista. As telas de status e o envio por WhatsApp passam a usar exatamente os dias realmente publicados, evitando diferença entre o que foi roteirizado e o que aparece como liberado.
